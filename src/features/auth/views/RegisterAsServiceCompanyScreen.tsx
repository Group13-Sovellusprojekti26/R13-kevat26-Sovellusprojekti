...
import { joinWithServiceCompanyInviteCode } from '../../../data/repositories/serviceCompanyInvites.repo';
...
type NavigationProp = NativeStackNavigationProp<AuthStackParamList, 'RegisterAsServiceCompany'>;
type RouteProps = RouteProp<AuthStackParamList, 'RegisterAsServiceCompany'>;

/**
 * Registration screen for service company using invite code
 * Shows pre-filled company data and collects personal info
 */
export const RegisterAsServiceCompanyScreen: React.FC = () => {
  const { t } = useTranslation();
  const navigation = useNavigation<NavigationProp>();
  const route = useRoute<RouteProps>();
  const { inviteCode, serviceCompanyData } = route.params;
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      // Register as service company user
      await joinWithServiceCompanyInviteCode({
        inviteCode,
        email: data.email,
      ...
      Alert.alert(
        ...
        t('auth.serviceCompanyRegistrationComplete'),
        [
          { 
            text: t('common.ok'),
            onPress: () => {
    ...
    <Screen scrollable>
      <View style={styles.container}>
        <Text variant="headlineMedium" style={styles.title}>
          {t('auth.createServiceCompanyAccount')}
        </Text>
        ...
        <Card style={styles.companyCard}>
          <Card.Content>
            <Text variant="titleMedium" style={styles.companyName}>
              {serviceCompanyData.housingCompanyName}
            </Text>
            <Text variant="bodyMedium" style={styles.companySubtitle}>
              {t('auth.servicingThisCompany')}
            </Text>
          </Card.Content>
        </Card>
        ...
        <Controller
          ...
          render={({ field: { onChange, onBlur, value } }) => (
            <TFTextField
              label={t('auth.serviceCompanyName')}
              value={value}
              onChangeText={onChange}
              onBlur={onBlur}
