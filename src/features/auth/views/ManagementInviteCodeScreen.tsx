...
import { validateManagementInviteCode } from '../../../data/repositories/managementInvites.repo';
...
type NavigationProp = NativeStackNavigationProp<AuthStackParamList, 'ManagementInviteCode'>;

/**
 * Screen for entering management invite code
 * Validates the code and navigates to registration form
 */
export const ManagementInviteCodeScreen: React.FC = () => {
  ...
  const onSubmit = async (data: InviteCodeFormData) => {
    ...
    try {
      const managementData = await validateManagementInviteCode(data.inviteCode.toUpperCase());
      haptic.success();
      
      // Navigate to registration with pre-filled data
      navigation.navigate('RegisterAsManagement', {
      ...
      <View style={styles.container}>
        ...
        <Surface style={styles.contentCard} elevation={1}>
          <Text variant="headlineMedium" style={styles.title}>
            {t('auth.enterManagementInviteCode')}
          </Text>
          <Text variant="bodyMedium" style={[styles.subtitle, { color: theme.colors.onSurfaceVariant }]}>
            {t('auth.managementInviteCodeSubtitle')}
          </Text>
          ...
          <Controller
            ...
            render={({ field: { onChange, onBlur, value } }) => (
              <TFTextField
                label={t('auth.managementInviteCode')}
                value={value}
