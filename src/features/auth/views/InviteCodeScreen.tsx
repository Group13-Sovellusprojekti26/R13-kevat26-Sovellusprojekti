...
import { validateInviteCode } from '../../../data/repositories/housingCompanies.repo';
...
type NavigationProp = NativeStackNavigationProp<AuthStackParamList, 'InviteCode'>;

/**
 * Screen for entering housing company invite code
...
export const InviteCodeScreen: React.FC = () => {
  ...
  const onSubmit = async (data: InviteCodeFormData) => {
    ...
    try {
      const companyData = await validateInviteCode(data.inviteCode.toUpperCase());
      haptic.success();
      
      // Navigate to registration with pre-filled data
      navigation.navigate('RegisterWithInvite', {
